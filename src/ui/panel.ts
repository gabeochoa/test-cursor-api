...
import type { CursorCloudClient, CloudAgentConversation, CloudAgentStatusResponse } from "../lib/cursorCloudClient";
import type { BmadWorkflow } from "../lib/workflowDiscovery";
import { getGitInfo } from "../lib/gitInfo";

type GetClient = () => Promise<CursorCloudClient>;

type UiState = {
  workflows: { name: string; description: string; workflowMdPath: string }[];
  ...
  prUrl?: string;
  runUrl?: string;
  conversation?: { messages: { id: string; type: string; text: string }[] };
};

export class BmadCloudPanel {
  private panel: vscode.WebviewPanel | undefined;
  private workflows: BmadWorkflow[] = [];
  private state: UiState = { workflows: [] };
  ...
  async setWorkflows(workflows: BmadWorkflow[]) {
  ...
  private async ensurePanel() {
    if (!this.panel) this.show();
  }

  private async bootstrapDefaults() {
    const git = await getGitInfo();
    if (!this.state.repoUrl) this.state.repoUrl = git.repositoryUrl;
    if (!this.state.ref) this.state.ref = git.ref;
  ...
  private async runWorkflowByName(name?: string) {
    if (!name) return;
  ...
  private async pollOnce(id: string) {
    try {
      const client = await this.getClient();
  ...
  private applyStatus(status: CloudAgentStatusResponse) {
  ...
  private applyConversation(convo: CloudAgentConversation) {
  ...
  private async sendFollowup(text: string) {
    const id = this.state.activeRunId;
    if (!id) return;
    const msg = text.trim();
    if (!msg) return;
